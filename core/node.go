...
//
// Nodes are the building blocks that composition helpers (such as [Then],
// [FanOut], and [Map]) combine into larger Nodes. Because a composite is itself a
// Node, a whole workflow is a single Node[I, O] that you Run.
//
// Implementations should be safe for concurrent use: the same Node value may be
// Run from multiple goroutines at once (for example inside [FanOut] or [Map]).
