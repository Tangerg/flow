...
// be observed, and it is why the reentry each subtest performs is the one its
// boundary's comment names as the risk.
func TestApplicationCodeMayReenterTheBoundaryThatCalledIt(t *testing.T) {
	increment := func(_ context.Context, value int) (int, error) { return value + 1, nil }
	seed := workflow.NewStore().WithOutput("seed", 1)

	t.Run("factory registers into the registry compiling it", func(t *testing.T) {
		...
		out, err := workflow.Run(t.Context(), step, seed, workflow.RunConfig{})
		if err != nil {
			t.Fatalf("Run: %v", err)
		}
		...
		// The registration took effect, which is what makes the reentry real
	...
	t.Run("emitter records into the run's journal", func(t *testing.T) {
		...
		_, err := workflow.Run(t.Context(), step, seed, workflow.RunConfig{
			Journal: journal,
			Emitter: workflow.EmitterFunc(func(_ context.Context, chunk workflow.Chunk) error {
				// The session's lock is held across this call. The Journal's is
				// not, and nothing holds both.
	...
	t.Run("observer reads the journal it is completing", func(t *testing.T) {
		journal := workflow.NewJournal()
		var visible []string
		_, err := workflow.Run(
			t.Context(),
		...
		// Reading the Journal from an Observer neither deadlocks nor comes too
		// early: a completion is emitted after its checkpoint is recorded, which
		// is what a host persisting the pair at this event depends on.
