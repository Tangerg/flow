...
func TestLoop_untilDone(t *testing.T) {
	...
	loop := workflow.Loop(workflow.LoopConfig{ID: "loop", Body: body, Condition: done, MaxIterations: 10})
...
// A Condition is the same node shape as a Resolver, so an ordinary typed
// pipeline can produce the stop decision without a condition-specific protocol,
...
func TestLoop_conditionComposesAsAnOrdinaryNode(t *testing.T) {
	...
	// Condition is an alias, so the composed pipeline already has that type:
	// there is no adapter and nothing to assert.
	done := flow.Then(read, reachedThree)

	body := workflow.LeafFunc("n", workflow.Output("n"),
		func(_ context.Context, value int) (int, error) { return value + 1, nil })
	out, err := workflow.Loop(workflow.LoopConfig{ID: "loop", Body: body, Condition: done}).
		Run(t.Context(), workflow.NewStore().WithOutput("n", 0))
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestLoop_conditionReadsIterationIndexFromScope(t *testing.T) {
	var seen []uint64
	body := workflow.LeafFunc("n", workflow.Output("n"),
		func(_ context.Context, value int) (int, error) { return value + 1, nil })
		...
		func(ctx context.Context, _ workflow.Store) (bool, error) {
			...
			if last.ID != "loop" || !last.Indexed {
				return false, fmt.Errorf("scope = %v; want an indexed loop frame", frames)
			}
			seen = append(seen, last.Index)
			return last.Index >= 2, nil
		},
	)
	if _, err := workflow.Loop(workflow.LoopConfig{ID: "loop", Body: body, Condition: done}).
		Run(t.Context(), workflow.NewStore().WithOutput("n", 0)); err != nil {
		t.Fatalf("Run: %v", err)
	}
	if !slices.Equal(seen, []uint64{0, 1, 2}) {
		t.Fatalf("iteration indexes = %v; want [0 1 2]", seen)
	}
}
...
func TestLoop_nilCondition(t *testing.T) {
	...
	// A Condition is the same node shape as a Resolver, so an absent one is a
	// nil node rather than a nil function.
	for name, done := range map[string]workflow.Condition{
		"nil interface":     nil,
		"typed nil adapter": flow.NodeFunc[workflow.Store, bool](nil),
	} {
...
func TestLoop_maxIterations(t *testing.T) {
	...
	done := flow.NodeFunc[workflow.Store, bool](func(context.Context, workflow.Store) (bool, error) { return false, nil }) // never done
...
func TestLoop_cancellationDuringBodyRollsBackIteration(t *testing.T) {
	...
	output, err := workflow.Loop(workflow.LoopConfig{ID: "loop", Body: body, Condition: flow.NodeFunc[workflow.Store, bool](func(context.Context, workflow.Store) (bool, error) {
		conditionRan = true
		return true, nil
	})}).
		Run(ctx, workflow.NewStore())
...
func TestLoop_cancellationDuringConditionRollsBackIteration(t *testing.T) {
	...
	step := workflow.Loop(workflow.LoopConfig{ID: "loop", Body: body, Condition: flow.NodeFunc[workflow.Store, bool](func(context.Context, workflow.Store) (bool, error) {
		cancel()
		return true, nil
	})})
...
func TestLoop_suspensionPreservesCompletedIterationWrites(t *testing.T) {
	...
	t.Run("condition", func(t *testing.T) {
		output, err := workflow.Loop(workflow.LoopConfig{ID: "loop", Body: write, Condition: flow.NodeFunc[workflow.Store, bool](func(context.Context, workflow.Store) (bool, error) {
...
func TestLoop_siblingBodiesWithTheSameIDHaveDistinctJournalScopes(t *testing.T) {
	...
	done := flow.NodeFunc[workflow.Store, bool](func(context.Context, workflow.Store) (bool, error) {
		return true, nil
	})
	pipeline := workflow.Sequence(
		workflow.Loop(workflow.LoopConfig{ID: "first", Body: body(&firstRuns, "FROM_FIRST"), Condition: done}),
...
func TestValidateSpec_siblingLoopBodiesMayReuseAnID(t *testing.T) {
	registry := workflow.NewRegistry().
		...
		MustRegisterCondition(
			"done",
			flow.NodeFunc[workflow.Store, bool](func(context.Context, workflow.Store) (bool, error) {
				return true, nil
			}),
		)
...
func TestLoop_rejectsInvalidStaticIdentities(t *testing.T) {
	...
	tests := map[string]workflow.Step{
		"loop ID collides before loop": workflow.Sequence(
			leaf("loop"),
			workflow.Loop(workflow.LoopConfig{ID: "loop", Body: leaf("body"), Condition: done}),
		),
...
func TestLoop_reportsJournalDecisionConflict(t *testing.T) {
	...
	), Condition: flow.NodeFunc[workflow.Store, bool](func(ctx context.Context, _ workflow.Store) (bool, error) {
