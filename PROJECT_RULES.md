...
- A generic operation lives on the concrete value that owns it. `Store.Get[T]`, `Ref.Bind[T]`, and
  `Expr.Eval[T]` keep typed behavior in the receiver's namespace; composition operators that combine
  peers — `Then`, `Map`, `Leaf`, `Factory` — stay package functions. Interfaces cannot declare
  generic methods, so `Node` and `Binder` keep one ordinary protocol method.
- `Store` owns every decision about its own representation. Callers ask a named method rather than
  reading `depth` or restating the overlay limit, and a composite that hands one Store to concurrent
  derivers passes it through `Store.sharedBase` first — `TestEveryFanOutSharesOneFlattenedBase`.
  Flattening is iterative, because graph width is not call-stack depth.
- Nesting is measured from what the walk already carries, never from a field raised on the way in and
  lowered on the way out — a stale raise turns a nesting limit into a size limit that only a wide
  input reveals — `TestCodec_boundsNestingNotBreadth`.
- Every exported `UnmarshalJSON` decodes through `jsondoc.DecodeInto`, so a nil receiver is reported
  rather than panicking and the destination is replaced only after complete success —
  `TestEveryUnmarshalJSONDecodesThroughOneBoundary`. A type that genuinely cannot route through it is
  named in `wireDecodeExceptions`.
- An exported result that is a slice or a map says what the caller owns, because a signature cannot.
...
- The axioms above are the design boundaries. When a change establishes a rule the next contributor
  would otherwise rediscover, state it here as a current-state axiom with the test that pins it —
  not as a record of what went wrong. Git history is the archaeology. The toolchain, the local gate,
  and what an exported change owes live in [CONTRIBUTING.md](./CONTRIBUTING.md).
